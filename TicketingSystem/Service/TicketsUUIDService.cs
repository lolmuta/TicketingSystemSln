...
{
    public class TicketsUUIDService
    {
        private readonly DbHelper dbHelper;

        public TicketsUUIDService(DbHelper dbHelper)
        {
        ...
        internal IEnumerable<TicketsInfo> GetTicketsInfoByPaidId(int paids_Id)
        {
            return dbHelper.ConnDb(conn =>
            {
                return conn.Query<TicketsInfo>(@"
                    SELECT [TicketsUUID].[UUID]
                          ,[TicketsUUID].[Paids_ID]
	                      , Acts.Title
	                      , Acts.Description
	                      , format(Acts_Date.Date,'yyyy/MM/dd HH:mm') as date
                      FROM [dbo].[TicketsUUID]
                      inner join Acts_Date on Acts_Date.ID = [TicketsUUID].Acts_Date_ID
                      inner join Acts on Acts.ID = Acts_Date.Acts_ID
                      where [Paids_ID] = @paids_Id
                ", new { paids_Id });
            });
        }
    }
